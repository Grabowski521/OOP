...
class TestInitLoggerMixin(unittest.TestCase):
    def test_product_creation(self):
        ...
        with StringIO() as buf, redirect_stdout(buf):
            ...
            expected = "Создан объект класса Product с параметрами: args=('Тестовый продукт', 'Описание', 1000, 10), kwargs={}"
    ...
    def test_smartphone_creation(self):
        ...
        with StringIO() as buf, redirect_stdout(buf):
            ...
            output = buf.getvalue().strip()
            expected = "Создан объект класса Smartphone с параметрами: args=('Тестовый смартфон', 'Описание', 5000, 5, 'Высокая', 'МодельY', 64, 'Синий'), kwargs={}"
            self.assertEqual(output, expected)
    ...
    def test_lawngrass_creation(self):
        ...
        with StringIO() as buf, redirect_stdout(buf):
            ...
            output = buf.getvalue().strip()
            expected = "Создан объект класса LawnGrass с параметрами: args=('Тестовая трава', 'Описание', 300, 15, 'Франция', '7 дней', 'Зелёный'), kwargs={}"
            self.assertEqual(output, expected)
